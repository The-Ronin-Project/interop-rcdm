...
class RoninPulseOximetryValidatorTest {
    ...
    private val flowRateCodeableConcept = CodeableConcept(
        coding = listOf(flowRateCoding),
        text = "FlowRate".asFHIR()
    )
    private val sourceFlowRateCodeExtension = Extension(
    ...
    private val concentrationCodeableConcept = CodeableConcept(
        coding = listOf(concentrationCoding),
        text = "Concentration".asFHIR()
    )
    private val sourceConcentrationCodeExtension = Extension(
        url = RoninExtension.TENANT_SOURCE_OBSERVATION_COMPONENT_CODE.uri,
        value = DynamicValue(
            DynamicValueType.CODEABLE_CONCEPT,
