...
class RoninDiagnosticReportNoteExchangeValidatorTest {
    private val validator = RoninDiagnosticReportNoteExchangeValidator()
    ...
    @Test
    fun `returns profile`() {
        assertEquals(RoninProfile.DIAGNOSTIC_REPORT_NOTE_EXCHANGE, validator.profile)
    }
    ...
    fun `validate fails if no category`() {
        val diagnosticReport = DiagnosticReport(
            id = Id("test-1234"),
            meta = Meta(profile = listOf(RoninProfile.DIAGNOSTIC_REPORT_NOTE_EXCHANGE.canonical)),
