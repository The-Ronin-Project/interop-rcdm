...
class RoninBodyTemperatureTransformerTest {
    private val bodyTemperatureCode = Code("8310-5")
    private val bodyTemperatureCodingList =
        listOf(
            Coding(
    ...
    private val bodyTemperatureConcept =
        CodeableConcept(
    ...
    private val registryClient =
        mockk<NormalizationRegistryClient> {
            ...
            } returns ValueSetList(bodyTemperatureCodingList, mockk())
        }

    private val transformer = RoninBodyTemperatureTransformer(registryClient)
    ...
    fun `returns correct profile`() {
        assertEquals(RoninProfile.OBSERVATION_BODY_TEMPERATURE, transformer.profile)
    }
    ...
    @Test
    fun `does not qualify when code coding code not for body temperature`() {
        val observation =
            Observation(
                id = Id("123"),
                status = com.projectronin.interop.fhir.r4.valueset.ObservationStatus.AMENDED.asCode(),
    ...
    @Test
    fun `does not qualify when code coding code is for body temperature but wrong system`() {
        val observation =
            Observation(
                id = Id("123"),
                status = com.projectronin.interop.fhir.r4.valueset.ObservationStatus.AMENDED.asCode(),
    ...
    fun `transform works`() {
        ...
        assertEquals(
            Meta(profile = listOf(Canonical(RoninProfile.OBSERVATION_BODY_TEMPERATURE.value)), source = Uri("source")),
            transformed.meta,
        )
