...
class RoninHeartRateTransformerTest {
    private val heartRateCode = Code("8867-4")
    private val heartRateCodingList =
        listOf(
            Coding(
    ...
    private val registryClient =
        mockk<NormalizationRegistryClient> {
            ...
            } returns ValueSetList(heartRateCodingList, mockk())
        }

    private val transformer = RoninHeartRateTransformer(registryClient)
    ...
    fun `returns correct profile`() {
        assertEquals(RoninProfile.OBSERVATION_HEART_RATE, transformer.profile)
    }
