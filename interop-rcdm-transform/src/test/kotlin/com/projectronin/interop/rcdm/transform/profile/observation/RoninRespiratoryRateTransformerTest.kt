...
class RoninRespiratoryRateTransformerTest {
    private val respiratoryRateCode = Code("9279-1")
    private val respiratoryRateCodingList =
        listOf(
            Coding(
    ...
    private val respiratoryRateConcept =
        CodeableConcept(
    ...
    private val registryClient =
        mockk<NormalizationRegistryClient> {
            ...
            } returns ValueSetList(respiratoryRateCodingList, mockk())
        }

    private val transformer = RoninRespiratoryRateTransformer(registryClient)
    ...
    fun `returns correct profile`() {
        assertEquals(RoninProfile.OBSERVATION_RESPIRATORY_RATE, transformer.profile)
    }
    ...
    @Test
    fun `does not qualify when code coding code not for respiratory rate`() {
        val observation =
            Observation(
                id = Id("123"),
                status = com.projectronin.interop.fhir.r4.valueset.ObservationStatus.AMENDED.asCode(),
    ...
    @Test
    fun `does not qualify when code coding code is for respiratory rate but wrong system`() {
        val observation =
            Observation(
                id = Id("123"),
                status = com.projectronin.interop.fhir.r4.valueset.ObservationStatus.AMENDED.asCode(),
    ...
    fun `transform works`() {
        ...
        assertEquals(
            Meta(profile = listOf(Canonical(RoninProfile.OBSERVATION_RESPIRATORY_RATE.value)), source = Uri("source")),
            transformed.meta,
        )
