...
class RoninBodyWeightTransformerTest {
    private val bodyWeightCode = Code("29463-7")
    private val bodyWeightCodingList =
        listOf(
            Coding(
    ...
    private val registryClient =
        mockk<NormalizationRegistryClient> {
            ...
            } returns ValueSetList(bodyWeightCodingList, mockk())
        }

    private val transformer = RoninBodyWeightTransformer(registryClient)
    ...
    fun `returns correct profile`() {
        assertEquals(RoninProfile.OBSERVATION_BODY_WEIGHT, transformer.profile)
    }
